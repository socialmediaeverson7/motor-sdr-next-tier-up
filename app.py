...
Foco 100% em Ollama Local para Independência de APIs
"""
...
# Renderizar sidebar e obter inputs (Focado em Ollama)
...
if st.session_state.processing:
    try:
        # 1. Inicializar IA (Focado em Ollama Local)
        with st.status(f"🧠 Conectando ao Ollama ({ollama_model})...", expanded=False) as status:
            ai_manager = validate_and_initialize_ai(ollama_model=ollama_model)
            if not ai_manager:
                st.session_state.processing = False
                st.stop()
            status.update(label="✅ IA Local Pronta!", state="complete")
        ...
        with ui.show_spinner("Extraindo lote de alvos..."):
            ...
            leads = get_leads_by_strategy(strategy_key, search_term=search_term, cnpj=cnpj_target)
        ...
        # 3. Processar lote com IA Local
        with ui.show_spinner(f"Processando cadências via Ollama ({ollama_model})..."):
        ...
        ui.show_error_message(f"Erro no processamento: {e}")
        st.session_state.processing = False
...
if email_button_clicked:
    send_email_safely(sender_email, sender_password, recipient_email, subject, body)
...
st.markdown(
    ...
    Motor SDR v2.0 | Independência Total via Ollama Local
    </div>
    """,
